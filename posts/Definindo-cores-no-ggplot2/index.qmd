...
Podemos referenciar as cores com a função _minhas_cores_ definida anteriormente. Aqui, utilizamos o atributo _fill_ de _geom_col_ para definir a cor de preenchimento das barras do gráfico.      
...
Através da função _scale_fill_manual_ podemos usar diferentes cores para cada espécie de pinguim.
...
e utilizar este array da @fig-named-colors com a função *scale_fill_manual*. As cores são atribuídas conforme nomeamos cada elemento do vetor de cores (por exemplo, "Adele = #641e16 (vermelho)". Se não tivéssemos feito isso, o sistema primeiro atribuiria as cores conforme a ordem alfabética como no código anterior e depois montaria o gráfico conforme a ordenação do _fct_reorder_.
...
Gostaríamos de modificar a escala de cores utilizando as cores previamente definidas em _minhas_cores_ e _minhas_paletas_.        
      
Primeiro precisamos definir uma função que cria uma customização da função _scale_color_continuous_.       
...
Vamos ajustar o último gráfico que fizemos para utilizar uma paleta de cores baseada na _minhas_paletas_("highlight").     
...
```
De forma similar à função _color_ que fizemos, podemos implementar uma função para _fill_:     
...
Este último gráfico não faz muito sentido, mas está aí apenas para exemplificar o uso da função _scale_fill_penguin_.       
