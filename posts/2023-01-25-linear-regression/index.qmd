...
Ao invés de simplesmente chamar a função *stan_glm* fizemos um monte de coisas. E qual a vantagem disto tudo ao invés de chamar o método diretamente?
...
-   Quando escrevemos **linear_reg()** estamos definindo o **tipo** do modelo (um tipo diferente seria, por exemplo, um random forest: *rand_forest()*).

-   **set_mode()** define o tipo de modelagem que será feita - as opções são tipicamente "classification" ou "regression".
...
Por fim, o pacote **broom** contém a função *tidy()* que permite retornar os parâmetros calculados do modelo num formato "tidy" (especificamente para modelos do pacote *rstanarm* temos que utilizar o método *tidy()* do pacote *broom.mixed*).
...
-   tidymodels separa a definição do modelo de seu cálculo (pacote *parsnip*):

    -   definição do modelo: *lienar_reg()*
    -   definição do modo do modelo: *set_mode("regression")*
    -   cálculo do modelo: *fit(...)*

<!-- -->

-   tidymodels permite acessar os resultados da regressão num formato tidy (pacote *broom*):
    -   *tidy(lm_fit)*

Este post implementou o primeiro exemplo do livro ROS utilizando tidymodels.
